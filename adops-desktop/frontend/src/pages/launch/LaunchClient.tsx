      ...
      {steps.map((label, i) => {
        ...
        return (
          <div key={n} className="flex items-center">
            <div className={`flex h-8 w-8 items-center justify-center rounded-full text-sm font-bold transition-colors ${done ? "bg-emerald-500 text-white" : active ? "bg-brand text-brand-fg" : "bg-raised text-muted"}`}>
              {done ? <CheckCircle size={16} /> : n}
            </div>
            <span className={`ml-2 text-sm font-semibold ${active ? "text-ink" : "text-muted"}`}>{label}</span>
            {i < steps.length - 1 && <div className="mx-3 h-px w-8 bg-stroke" />}
          </div>
        );
      })}
    </div>
...
function StructureCard({
  ...
  return (
    <div
      onClick={onClick}
      className={`cursor-pointer rounded-lg border-2 p-4 transition-all ${selected ? "border-brand bg-selected" : "border-stroke bg-card hover:border-brand"}`}
    >
      <div className="flex items-start justify-between gap-2">
        ...
        <div className={`mt-0.5 h-4 w-4 flex-shrink-0 rounded-full border-2 ${selected ? "border-brand bg-brand" : "border-stroke"}`} />
      </div>
              ...
              <table className="w-full min-w-[700px] text-left text-sm">
                ...
                <tbody>
                  {accounts.map((acc) => {
                    ...
                    return (
                      <tr key={acc.id}
                        className={`cursor-pointer border-t border-stroke ${sel ? "bg-selected" : "hover:bg-raised"}`}
        ...
        <div className="space-y-4">
          ...
          {rules.structure === "Z_GROUPED" && (
            <Card className="border-blue-200 bg-blue-50/40">
              ...
              {showKeitaroForm && (
                <div className="mb-4 grid gap-3 rounded-lg bg-card p-3 shadow-card md:grid-cols-2">
          ...
          <Card>
            ...
            <p className="mb-3 text-xs text-muted">Переменные: <code className="rounded bg-raised px-1">{"{account}"}</code> <code className="rounded bg-raised px-1">{"{creative}"}</code> <code className="rounded bg-raised px-1">{"{date}"}</code> <code className="rounded bg-raised px-1">{"{objective}"}</code></p>
            <div className="grid gap-3 md:grid-cols-3">
              <div className="space-y-1">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {templates.map((t) => (
                    <button key={t.id} onClick={() => setRules({ ...rules, objective: t.objective, campaignStatus: t.campaignStatus, dailyBudget: t.dailyBudget ? String(t.dailyBudget) : "", bidStrategy: t.bidStrategy, optimizationGoal: t.optimizationGoal })}
                      className="rounded border border-stroke bg-raised px-2 py-1 text-xs font-semibold text-ink hover:bg-selected">
                      {t.name}
                    </button>
                  ))}
                </div>
      ...
      {step === 3 && (
        <div className="space-y-3">
          <Card>
            ...
            {showCreativeForm && (
              <div className="mt-3 grid gap-3 rounded-lg bg-raised p-3 md:grid-cols-2">
                ...
                <textarea placeholder="Основной текст" value={newCreative.primaryText}
                  ...
                  rows={5} className="w-full rounded border border-stroke bg-card px-3 py-2 text-[13px] text-ink outline-none placeholder:text-muted focus:border-brand focus:ring-1 focus:ring-brand" />
                <div className="flex gap-2 md:col-span-2">
              ...
              {creatives.map((c) => {
                ...
                return (
                  <div key={c.id} onClick={() => setSelectedCreativeIds((prev) => sel ? prev.filter((id) => id !== c.id) : [...prev, c.id])}
                    className={`cursor-pointer rounded-lg border p-3 transition-all ${sel ? "border-brand bg-selected ring-2 ring-brand/20" : "border-stroke bg-card hover:border-muted"}`}>
                    <div className="flex items-start justify-between gap-2">
          ...
          <Card>
            ...
            <div className="grid grid-cols-2 gap-3 md:grid-cols-5">
              <div className="rounded-lg bg-raised p-3 text-center">
                <div className="text-xl font-black text-ink">{selectedAccountIds.length}</div>
                <div className="text-xs text-muted">Кабинетов</div>
              </div>
              <div className="flex items-center justify-center text-muted text-xl font-bold">×</div>
              <div className="rounded-lg bg-raised p-3 text-center">
                <div className="text-xl font-black text-ink">{selectedCreativeIds.length}</div>
                <div className="text-xs text-muted">Креативов</div>
              </div>
              <div className="flex items-center justify-center text-muted text-xl font-bold">=</div>
              <div className="rounded-lg bg-selected p-3 text-center ring-2 ring-brand/30">
                <div className="text-xl font-black text-brand">{totals.ads}</div>
                <div className="text-xs text-brand">Объявлений</div>
              </div>
            </div>
