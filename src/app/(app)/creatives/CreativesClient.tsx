...
type Creative = {
  id: string; name: string; type: string; zGroup: string | null; geo: string | null;
  headline: string | null; primaryText: string | null;
  callToAction: string; destinationUrl: string | null; mediaUrl: string | null;
  createdAt: string;
};
...
export function CreativesClient() {
  ...
  const [bulkFiles, setBulkFiles] = useState<{ name: string; zGroup: string | null; geo: string | null; url: string }[]>([]);
  ...
  async function save() {
    ...
    await fetch("/api/creatives", {
      ...
      body: JSON.stringify({
        ...form,
        zGroup: parseCreativeFilename(form.name).zNum ?? null,
        geo: parseCreativeFilename(form.name).geo ?? null,
      ...
      {showForm && (
        <Card>
          ...
          <p className="mb-3 text-xs text-slate-500">
            Гео и ключ заголовка определяются из названия: <code className="rounded bg-slate-100 px-1">SKANDAL-ES-1805-2-Z1.mp4</code> → гео <Badge tone="neutral">ES</Badge> заголовок <Badge tone="good">Z1</Badge>
          </p>
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <textarea placeholder="Основной текст объявления"
                ...
                rows={5}
                className="w-full rounded-md border border-line bg-white px-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" />
            </div>
          </div>
