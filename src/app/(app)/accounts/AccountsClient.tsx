...
export function AccountsClient() {
  ...
  async function load() {
    ...
    const [accountsRes, poolsRes] = await Promise.all([fetch(`/api/accounts?${params}`), fetch("/api/account-pools")]);
    setAccounts((await accountsRes.json()).accounts ?? []);
    setPools((await poolsRes.json()).pools ?? []);
    setLoading(false);
  }
  ...
  async function syncMock() {
    await fetch("/api/integrations/meta/sync", { method: "POST" });
    await load();
  }

  async function healthCheck(ids: string[]) {
    if (!ids.length) return;
    await fetch("/api/accounts/bulk-health-check", {
      method: "POST",
  ...
  async function addToPool() {
    ...
    await fetch(`/api/account-pools/${targetPoolId}/items`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
            ...
            <tbody>
              {accounts.map((account) => (
                <tr key={account.id} className="border-t border-line">
                  ...
                  <td>{account.spendLimit ? money(Number(account.spendLimit), account.currency) : "-"}</td>
                  <td className="max-w-[220px]">
                    <div className="flex flex-wrap gap-1">
                      {account.pools.length ? account.pools.map(({ pool }) => <span key={pool.id} className="rounded px-2 py-0.5 text-xs font-bold" style={{ backgroundColor: `${pool.color}16`, color: pool.color }}>{pool.name}</span>) : "-"}
                    </div>
                  </td>
