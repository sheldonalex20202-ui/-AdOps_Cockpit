...
function LoginForm() {
  ...
  const [error, setError] = useState(
    oauthError === "oauth_failed" ? "Ошибка OAuth авторизации. Попробуйте снова." : ""
  );
  ...
  const registerHref = `/register${callback ? `?callback=${encodeURIComponent(callback)}${state ? `&state=${encodeURIComponent(state)}` : ""}` : ""}`;
        ...
        <div className="relative backdrop-blur-xl bg-white/10 border border-white/10 rounded-3xl p-8 shadow-2xl">
          ...
          <div className="text-center mt-6">
            <span className="text-white/60 text-sm">
              Нет аккаунта?{" "}
              <Link href={registerHref} className="text-blue-400 font-medium hover:text-blue-300 transition-colors">
                Зарегистрироваться
              </Link>
            </span>
          </div>
