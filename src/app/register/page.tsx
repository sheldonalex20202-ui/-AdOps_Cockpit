    ...
    <main className="flex min-h-screen flex-col items-center justify-center bg-zinc-950 px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-6">
          <h2 className="mb-5 text-sm font-semibold text-zinc-300">Создать аккаунт</h2>
          ...
          <p className="mt-4 text-center text-xs text-zinc-500">
            Уже есть аккаунт?{" "}
            <Link
              href={`/login${callback ? `?callback=${encodeURIComponent(callback)}${state ? `&state=${encodeURIComponent(state)}` : ""}` : ""}`}
        ...
        <p className="mt-4 text-center text-xs text-zinc-600">
          ...
          <Link href="/terms" className="text-zinc-500 hover:text-zinc-400">условиями</Link>
        </p>
      </div>
    </main>
  );
