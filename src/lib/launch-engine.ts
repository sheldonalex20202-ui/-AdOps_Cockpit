...
export async function runLaunchJob(
  ...
  const items = await prisma.launchJobItem.findMany({
    where: { launchJobId: jobId },
  ...
  for (const item of items) {
    ...
    if (success) {
      ...
      if (structure === "CBO") resultJson = buildCBO(item.adAccount.name, creatives, cfg);
      else if (structure === "ABO") resultJson = buildABO(item.adAccount.name, creatives, cfg);
      else if (structure === "ISOLATION") resultJson = buildISOLATION(item.adAccount.name, creatives, cfg);
      else resultJson = buildZGrouped(item.adAccount.name, creatives, cfg, headlines, campaignsPerAccount);

      await prisma.launchJobItem.update({
        where: { id: item.id },
        data: { status: "SUCCESS", resultJson, updatedAt: new Date() },
      });
      successCount++;
    } else {
      await prisma.launchJobItem.update({
        ...
        data: {
          status: "FAILED",
          errorMessage: MOCK_ERRORS[Math.floor(Math.random() * MOCK_ERRORS.length)],
          updatedAt: new Date(),
        },
      });
      failedCount++;
  ...
  await prisma.launchJob.update({
    where: { id: jobId },
    data: { status: finalStatus, successCount, failedCount, completedAt: new Date(), updatedAt: new Date() },
  });
}
